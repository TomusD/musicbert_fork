...
class MuMoE(nn.Module):
    def __init__(self, MuMoE_layer, input_dim, output_dim, expert_dims, ranks, second_dim=0, normalization='bn', act=entmax, use_bias=True, hierarchy=1, apply_ln=False):
        ...
        if self.normalization == 'ln': self.ln = nn.ModuleList([torch.nn.LayerNorm(dim, elementwise_affine=False) for dim in expert_dims])
...
class MuMoETransformerLayer(nn.Module):
    """
    (Triantafyllou)
    A TransformerEncoderLayer where the FFN is replaced by a MuMoE block.
    """
    # TODO: FIX -fp16 compatibility
    def __init__(self, original_layer, mumoe_params):
        super().__init__()
        self.self_attn = original_layer.self_attn
        self.dropout_module = original_layer.dropout_module
        self.self_attn_layer_norm = original_layer.self_attn_layer_norm
        self.final_layer_norm = original_layer.final_layer_norm

        input_dim = original_layer.fc1.in_features
        out_dim = original_layer.fc1.out_features
        
        original_params = (original_layer.fc1.in_features * original_layer.fc1.out_features) + \
                          (original_layer.fc2.in_features * original_layer.fc2.out_features)
        if original_layer.fc1.bias is not None:
            original_params += original_layer.fc1.out_features + original_layer.fc2.out_features
        
        print(f"Original FFN parameter budget: {original_params:,}")

        num_experts = mumoe_params['num_experts']
        gating_params = input_dim * num_experts

        # Calculate the number of ranks based on the original parameters and the gating parameters
        if mumoe_params['moe_method'] == 'CP':
            moe_class = CPMuMoE
            per_rank_cost = (num_experts + input_dim + out_dim) + (num_experts + out_dim + input_dim)
        ...
        self.ffn_block = MuMoE(
            MuMoE_layer=moe_class,
            input_dim=input_dim,
            output_dim=out_dim,
            second_dim=input_dim,
            expert_dims=[num_experts],
            ranks=ranks,
            normalization='ln', 
            hierarchy=1,
            use_bias=True
    ...
    def forward(self, x, self_attn_padding_mask=None, attn_mask=None):
        # Self-attention
        residual = x
...
# Replaces standard TransformerEncoderLayer instances with the new MuMoETransformerLayer.
def inject_mumoe(model, target_layer_indices, mumoe_params):
    print(f"Replacing layers {target_layer_indices} with MuMoETransformerLayer")
    transformer_layers = model.encoder.sentence_encoder.layers
    for layer_idx in target_layer_indices:
        original_layer = transformer_layers[layer_idx]
        new_layer = MuMoETransformerLayer(original_layer, mumoe_params)
        transformer_layers[layer_idx] = new_layer
    print("MuMoE layer replacement complete.")